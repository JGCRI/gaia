...
The example below uses the monthly precipitation and temperature projections from 2015 to 2100. To run this climate data processing, please provide the file paths for the precipitation and temperature NetCDF files using the `pr_ncdf` and `tas_ncdf` arguments, respectively, and adjust the other arguments accordingly to match the specifics of your climate data.
...
* `historic_vars_[crop-name].csv`: This file summarizes historical harvest area, irrigation type, yield, CO2, growing season, weather data by country and crop. For example, historical information for soybean is illustrated in [Table 4](#table4).

* `weather_[climate-model]_[climate-scenario]_[crop-name].csv`: This file includes future aggregated weather data for the specified climate model/climate scenario/crop. An example of climate information for soybean is shown in [Table 5](#table5).
