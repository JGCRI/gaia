...
1. [`weighted_climate`](#weighted_climate): Processes CMIP6 daily or monthly climate NetCDF data formatted in accordance with the [ISIMIP simulation protocols](https://www.isimip.org/protocol/preparing-simulation-files/ "Here we specifically require the bias-adjusted and statistically downscaled CMIP climate data following ISIMIP's protocols") (more details [here](#note3)) and calculates cropland-weighted precipitation and temperature at the country level, differentiated by crop type and irrigation type.
...
* Format your data according to ISIMIP's guidelines for [preparing simulation files](https://www.isimip.org/protocol/preparing-simulation-files/) in terms of NetCDF headers, grid format, variables and dimensions, and the time axis. More tips can be found [here](#note3).</p>
</div>
...
If you have your own climate forcing data, please follow the guidance below to format your data to be compatible with `gaia`.

<a name="note3"></a>
<details>
<summary>**Tips for Formatting Climate Forcing NetCDF (Click to Expand the Content)**</summary>

<div class="alert alert-info">
<h4 class="alert-heading">Formatting Your Climate Forcing Data for gaia</h4>
  <p class="mb-0">
  `gaia` requires that the climate NetCDF files follow the basic formatting rules, specifically in terms of variables and dimensions, spatial resolution, and attributes instructed in [ISIMIP protocols](https://www.isimip.org/protocol/preparing-simulation-files). The followings are some of the [general formatting requirements](https://www.isimip.org/protocol/preparing-simulation-files/#general-formatting-requirements) from ISIMIP that are compatible with `gaia`.
  
**Variables and Dimensions**

 * Variable names for precipitation and temperature should be `pr` and `tas`, respectively.
 * Every dimension should have an associated coordinate variable. For `pr` and `tas`, they should include three dimensions (coordinate variables): `time`, `lon`, and `lat`.
 ...
 * Each variable's attribute should at least include the units. `pr` should be in the unit of *kg m-2 s-1*, and `tas` should be in the unit of *K*.
...
Both [Example 1](#example-1) or [Example 2](#example-2) will generate intermediate and final outputs. Explore the results in your output folder. For a detailed explanation of the outputs, checkout [Explore Outputs](#explore-outputs) section!

<br />

# Explore Outputs

[Example 1](#example-1) and [Example 2](#example-2) demonstrate how to use the wrapper function `yield_impact` to streamline the entire workflows shown in [Figure 1](#figure1).

In this Section, we provide additional details on using each function described in the [Workflow Overview](#workflow-overview) section with the example data and explain the associated outputs and diagnostic plots if any. **This section can help you better understand the workflow and the structure of the outputs, and allow you to make changes to certain steps (e.g., crop calendars) if needed.**

If you have run [Example 1](#example-1) or [Example 2](#example-2) successfully, you should have all the outputs available. However, you may choose to run the code snippet within each subsection to regenerate these outputs if you prefer. Please note that running the following code snippets will overwrite the existing outputs in the corresponding `output_dir`.
...
Please note that we have excluded countries with `cassava` planting seasons longer than two months, based on SAGE data. This is because extended planting periods from SAGE data typically indicate multiple planting and harvesting cycles, which makes it difficult to identify yield for each individual harvest.
...
The current version of `gaia` has several limitations that users should be aware of.

## Weather Data Aggregation and Theoretical Consistency

In the current implementation, `gaia` uses cropland area-weighted averages of weather variables (e.g., temperature and precipitation) to construct country-level climate inputs. While this aggregation method is commonly used in empirical crop yield modeling, its theoretical justification is more robust when the dependent variable (e.g., yield) is in linear form, as it can be interpreted as a consistent approximation of a field-level data-generating process.

However, in `gaia`, the dependent variable is specified as the natural logarithm of yield, which allows us to project the future yield shocks based on weather only, allowing modelers to use their own assumptions about technological improvements driving yield improvements. In this context, the use of cropland-weighted average climate inputs might lead to theoretical inconsistency. Specifically, if yield is assumed to be generated by an underlying nonlinear function of weather at the field level (e.g., $$log(y)=f(T,P)$$), then aggregating climate variables before applying the function is not equivalent to aggregating yield outcomes after applying the function at the field level. This is a known issue related to the log-sum-exp transformation.

While this limitation does not undermine the empirical validity of the model, users should be aware that the aggregation procedure may not be interpretable as a field-level yield formulation. The software currently supports only cropland area-weighted climate and does not offer alternative methods for constructing climate inputs.

<br />

## Uncertainty Quantification

`gaia` currently does not explicitly quantify uncertainty in projected yield shock outcomes resulting from uncertainty in the estimated econometric coefficients. While `gaia` provides statistics of the regression analysis, it does not perform uncertainty quantification, such as Monte Carlo simulations, to propagate the uncertainty in model parameters through to the final results. However, users are provided with all necessary outputs (including coefficient estimates and associated statistics in the reg_out_[crop]_xxx.csv files) to implement such an analysis independently, if desired.
