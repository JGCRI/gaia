                       ...
                       crop_calendar = NULL)
{
  ...
  d <- subset( d, select = c( "iso", "year", "crop", "irr_rf", "var", "year1", "year2", "plant", "harvest",
...
#' data_trans
#'
#' Create dataframe with future monthly weather variables and iso codes for
...
prep_regression <- function(data = NULL)
{
  ...
  d1 <- subset( d1, select = c( "iso", "year", "gdp_pcap_ppp", "crop", "area_harvest", "irr_equip",
  ...
  d2 <- subset( d2, select = c( "iso", "year", "gdp_pcap_ppp", "crop", "area_harvest", "irr_equip",
                           ...
                           output_dir = file.path(getwd(), 'output'))
{
  ...
  for(group in groups){
    df <- d %>%
      dplyr::filter(GCAM_region_name == group$GCAM_region_name,
                    iso == group$iso) %>%
  ...
  p <- ggplot2::ggplot( data, ggplot2::aes( x = year, y = yield_impact, color = iso ) ) +
    ...
    ggplot2::labs(x = "year",
                  ...
                  title = paste( climate_model, "|", climate_scenario, "| temp and precip impacts on",
                                 crop_name, "yields by country, relative to ", base_year, "_avg", sep = " " )) +
    theme_basic +
    ggplot2::theme(
